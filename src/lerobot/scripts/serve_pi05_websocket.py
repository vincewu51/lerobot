"""
WebSocket server adapter for LeRobot Pi05 policy to work with OmniGibson/Isaac Sim.

This script loads a LeRobot Pi05 model and serves it via WebSocket protocol,
compatible with OmniGibson's WebsocketClientPolicy interface.

Usage:
    python src/lerobot/scripts/serve_pi05_websocket.py \
        --pretrained_name_or_path=~/workspace/pi05-task0000/10000 \
        --device=cuda \
        --host=0.0.0.0 \
        --port=8000 \
...
@dataclass
class ServerConfig:
    """Configuration for Pi05 WebSocket Server"""
    pretrained_name_or_path: str
    policy_type: str = "pi05"
    device: str = "cuda"
    host: str = "0.0.0.0"
    port: int = 8000
    task: str = "Turn on the radio receiver that's on the table in the living room."  # Task description for Pi05
    # Image features expected by policy (should match training config)
    image_features: Optional[list] = None


class Pi05WebSocketServer:
    """Serves LeRobot Pi05 policy via WebSocket protocol compatible with OmniGibson"""
    ...
    def __init__(self, config: ServerConfig):
        ...
        # Pi05 loads dataset stats automatically from the checkpoint directory
        # Unlike SmolVLA, it doesn't need stats to be passed explicitly
        self.policy = policy_class.from_pretrained(str(model_path))
    ...
    def reset(self):
        ...
        # Pi05 policies may have internal state for action chunking. Reset here if needed.
    ...
    def preprocess_observation(self, obs: Dict[str, np.ndarray]) -> Dict[str, torch.Tensor]:
        """
        Convert OmniGibson observation format to LeRobot format for Pi05 model.
        ...
        - task: string (task description for Pi05)
        """
        lerobot_obs = {}
    ...
    async def run(self):
        ...
        logger.info(f"Starting Pi05 WebSocket server on {self.config.host}:{self.config.port}...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Serve LeRobot Pi05 via WebSocket")
    parser.add_argument("--pretrained_name_or_path", type=str, required=True,
                       help="Path to pretrained Pi05 model")
    parser.add_argument("--policy_type", type=str, default="pi05",
                       help="Policy type (default: pi05)")
    ...
    parser.add_argument("--task", type=str, default="Turn on the radio receiver that's on the table in the living room.",
                       help="Task description for Pi05 (default: 'Turn on the radio receiver that's on the table in the living room.')")
    ...
    server = Pi05WebSocketServer(config)
